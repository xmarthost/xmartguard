...
| WAF Rule Sets | One ModSecurity configuration for all servers from the portal: XMart Guard rules, **OWASP Core Rule Set** (latest official release, downloaded by the portal), cPanel vendors such as **Malware.Expert** (paid) or Comodo added and auto-updated through WHM, and custom rules; per-server rollout status. Hits are read from the error log and the ModSecurity audit log (as in WHM » ModSecurity Tools). See [docs/WAF-RULESETS.md](docs/WAF-RULESETS.md) |
...
| Portal | Works on phones (card lists, stacked settings, no sideways scrolling); cPGuard-style server cards (virus/web attacks, IPDB sparkline, blacklists, domains), live IPDB chart with sliding new entries, collapsible sidebar with group flyouts, server dashboard with **protection status** (realtime scanner, firewall, IPDB, WAF: running / not running alerts), Knowledge Base with setup guides and third-party attributions, Firewall Logs with flags/CSV, Mass Operations, roles, audit log |
